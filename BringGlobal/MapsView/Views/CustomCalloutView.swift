...
class CustomCalloutView: CalloutView {
    ...
    private func configure() {
        ...
        let vflStrings = [
            "V:|-[titleLabel]-[subtitleLabel]-[bookmarkButton]-|",
            "H:|-[titleLabel]-|",
            "H:|-[subtitleLabel]-|",
            "H:|-[bookmarkButton]-|"
        ]
    ...
    @objc func didTapBookmarkButton(_ sender: UIButton) {
        if let mapView = mapView, let delegate = mapView.delegate as? CustomCalloutViewDelegate {
            delegate.mapView(mapView, didTapBookmarkButton: sender, for: annotation!)
        }
    }
